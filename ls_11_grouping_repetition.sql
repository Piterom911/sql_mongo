...
select min(price), max(price), category_id
from products
group by category_id;
...
select cat.category_name, avg(prod.price) as average_price
...
-- Вывести названия товаров, которые стоят дороже средней стоимости всех товаров, но находятся в категории, где средняя цена ниже 100.

select prod.category_id, prod.product_name
from products as prod
join ( select category_id, avg(price) as avg_price
from products as prod
group by category_id
