...
db.workers.updateOne({ _id: 2 }, { $set: { position: "Sales manager" } });
// mysql
// update workers
...
db.workers.updateOne({ _id: 2 }, { $set: { department: "Sales" } });
//mysql
// alter table workers
...
db.workers.updateOne({ _id: 2 }, { $unset: { department: 1 } });
...
//ERROR!  The field 'position' must be an array but is of type string in document {_id: 2}
...
db.workers.updateMany({}, { $push: { skills: "Javascript" } });
// Всем сотрудникам добавить новый навык 'Javascript', если такого навыка у них нет.
db.workers.updateMany({}, { $addToSet: { skills: "Javascript" } });
...
db.workers.updateOne(
  ...
  {
    $addToSet: {
      skills: { $each: ["teamwork", "programming", "seating guests"] },
    },
  }
);
...
db.workers.updateOne(
  ...
  {
    $push: {
      skills: { $each: ["budgeting", "organization"], $position: 0, $slice: 4 },
    },
  }
);

// Удалить первый навык сотрудника Marina
db.workers.updateOne({ firstname: "Marina" }, { $pop: { skills: -1 } });
// Удалить последний навык сотрудника Boris
db.workers.updateOne({ firstname: "Boris" }, { $pop: { skills: 1 } });

// Удалить навык coding у Boris.
db.workers.updateOne({ firstname: "Boris" }, { $pull: { skills: "coding" } });
// Удалить навык 'Javascript' у всех сотрудников.
db.workers.updateMany({}, { $pull: { skills: "Javascript" } });
